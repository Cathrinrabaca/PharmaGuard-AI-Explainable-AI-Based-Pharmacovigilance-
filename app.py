...
DATA_PART1 = os.path.join(BASE_DIR, "data", "dataset_part1.csv")
DATA_PART2 = os.path.join(BASE_DIR, "data", "dataset_part2.csv")

@st.cache_data(show_spinner="Loading dataset ...")
def load_data():
    if os.path.exists(DATA_PATH):
        return pd.read_csv(DATA_PATH)
    # Load from split files (used on Streamlit Cloud)
    df = pd.concat([pd.read_csv(DATA_PART1), pd.read_csv(DATA_PART2)], ignore_index=True)
    return df
